...
const onTimeUpdate = (newTime: string | null) => {
  if (newTime) {
    ...
    try {
      ...
      // Try to parse the time using date-fns
      const parsedTime = parse(newTime, 'h:mm a', baseDate)

      if (isValid(parsedTime)) {
        console.log('Valid time from picker:', parsedTime.toLocaleTimeString())
        ...
        // Then emit the change to parent
        emit('update:model-value', newDateIso)

        // Double-check our values after the update to ensure they weren't reset
        setTimeout(() => {
          if (tempTime.value !== newTime) {
            console.warn('Time got reset by watcher! Forcing back to:', newTime)
            tempTime.value = newTime
            editableTime.value = newTime
          }
        }, 10)
...
const updateDateTime = () => {
  try {
    ...
    // Start with the current date to preserve it
    const currentDateObj = new Date(date.value || new Date())
    let baseDate: Date

    // Process the date part if we have it
    if (tempDate.value) {
      try {
        // Parse the date from tempDate (YYYY-MM-DD format)
        const parsedDate = parse(tempDate.value, 'yyyy-MM-dd', new Date())

        if (isValid(parsedDate)) {
          // Keep the existing time but update the date components
          baseDate = new Date(currentDateObj)
          baseDate.setFullYear(parsedDate.getFullYear())
          baseDate.setMonth(parsedDate.getMonth())
          baseDate.setDate(parsedDate.getDate())
        } else {
          baseDate = new Date(currentDateObj)
        }
      } catch (e) {
      ...
      // Keep the existing date if no new date is specified
      baseDate = new Date(currentDateObj)
    }

    // Process the time part if we have it
    if (tempTime.value) {
      console.log('Processing time from tempTime:', tempTime.value)

      try {
        // Try parsing with date-fns for the time part only
        const timeParts = parse(tempTime.value, 'h:mm a', new Date())

        // Transfer hours and minutes to our base date
        if (isValid(timeParts)) {
          console.log('Valid time parsed:', timeParts.toLocaleTimeString())
          baseDate.setHours(timeParts.getHours(), timeParts.getMinutes(), 0, 0)
        } else {
          console.warn('Failed to parse time with date-fns, trying regex')
        }
      } catch (e) {
        console.error('Error with date-fns parsing:', e)

        // Fallback: Simple regex-based parsing
        const timeMatch = tempTime.value.match(/(\d{1,2}):(\d{2})\s*(am|pm)/i)
        if (timeMatch) {
          console.log('Regex time match:', timeMatch)
    ...
    console.log('Final date/time to emit:', baseDate.toISOString())

    // Update the model with the new date/time
    date.value = baseDate.toISOString()
    emit('update:model-value', date.value)
  } catch (e) {
