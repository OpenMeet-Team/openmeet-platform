...
<template>
    ...
    <q-img class="cursor-pointer rounded-borders" style="max-width: 300px; height: 150px;" @click="navigateToEvent(event.slug, event.id)" ratio="16/9" :src="getImageSrc(event.image)">
      <div class="q-pa-none absolute q-ml-sm no-padding bg-transparent"><q-badge>{{ event.type }}</q-badge></div>
    </q-img>
    <div class="col column">
      <div class="text-h5 text-bold q-pa-none cursor-pointer elipsys" @click="navigateToEvent(event.slug, event.id)">{{ formatDate(event.startDate) }}</div>
