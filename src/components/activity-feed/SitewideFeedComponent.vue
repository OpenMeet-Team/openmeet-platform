...
async function fetchActivities () {
  ...
  error.value = null
  try {
    logger.debug('Fetching sitewide activity feed')
    const response = await activityFeedApi.getSitewideFeed({ limit })
    logger.debug(`Sitewide feed response: ${response.data.length} items`)
    ...
    logger.error('Failed to load sitewide activity feed', {
...
async function loadMore () {
  ...
  try {
    ...
    const response = await activityFeedApi.getSitewideFeed({
      limit,
    ...
    logger.error('Failed to load more sitewide activities', {
      error: err
    })
  } finally {
...
function formatActivityText (activity: ActivityFeedEntity): {
  ...
  if (activityType === 'group.created') {
    return {
      text: `${metadata.actorName} created ${metadata.groupName}`,
  ...
  if (activityType === 'group.milestone') {
    ...
    return {
      text: `${metadata.groupName} reached ${value} members! 🎉`,
      groupLink: metadata.groupSlug
    }
  }
...
function navigateToGroup (groupSlug: string, event: Event) {
    ...
    <q-card flat>
      ...
      <q-list v-else-if="activities.length" class="activity-list" padding>
        ...
        <q-card-section v-else-if="activities.length > 0" class="text-center">
          <div class="end-message">That's all the recent activity</div>
        </q-card-section>
      </q-list>
