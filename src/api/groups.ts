...
export const groupsApi = {
  ...
  sendAdminMessage: (slug: string, data: { subject: string, message: string }): Promise<AxiosResponse<{ success: boolean, deliveredCount: number, failedCount: number, messageId: string }>> => api.post(`/api/groups/${slug}/admin-message`, data, createGroupApiHeaders(slug)),
  previewAdminMessage: (slug: string, data: { subject: string, message: string, testEmail: string }): Promise<AxiosResponse<{ message: string }>> => api.post(`/api/groups/${slug}/admin-message/preview`, data, createGroupApiHeaders(slug))
}
